...
*  \author     K
*
*  \Tested with Linux raspberrypi 5.4.83-v7l+
*
* *******************************************************************************/
#include <linux/module.h>// Đây là nơi module_initvà module_exitđược xác định. module_initcho kernel biết đâu là điểm vào chương trình của chúng ta và cũng chỉ có một module_initlệnh gọi cho chương trình mà chúng ta viết
#include <linux/init.h>//gồm tất cả những thứ liên quan đến __init
#include <linux/slab.h>
#include <linux/i2c.h> //definitions for the Linux i2c bus interface
#include <linux/delay.h>
#include <linux/kernel.h>//liệt kê tất cả các tính năng xảy ra khối lượng công việc
...
*/ 
// cấu hình cho từng kí tự riêng biệt trong ssd1306 hiện theo pixel 
...
** hàm ghi data vào trong máy client trong giao thức I2C
**  Arguments:
**      buff -> buffer to be sent
**      len  -> Length of the data
**   
*/
static int I2C_Write(unsigned char *buf, unsigned int len)// bit r/w sẽ ở mức thấp
{
  /*
...
** 
*/
static int I2C_Read(unsigned char *out_buf, unsigned int len)//bit r/w sẽ ở mức cao
{
  /*
  ** Sending Start condition, Slave address with R/W bit, 
  ** ACK/NACK and Stop condtions will be handled internally.
...
static void SSD1306_Write(bool is_cmd, unsigned char data)
{
  ...
  */ 
  ...
  {
      buf[0] = 0x40; // so hex 40 chuyen sang nhi phan 0010 0000
  }
...
static void SSD1306_InvertDisplay(bool need_to_invert)
{
  if(need_to_invert)
  {
    SSD1306_Write(true, 0xA7); // Invert the display đảo ngược led mức 0 sáng, led 1 là tắt
  }
  else
  {
    SSD1306_Write(true, 0xA6); // Normal display giữ nguyên led 0 tắt, led 1 sáng
  }
}
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("nhukhoi3004@gmail.com>");
